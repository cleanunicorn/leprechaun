...
(function() {
  var KrakenClient, MA, buy, buy_price, check_moving_average, fee_percent, get_market_price, invested, kraken, kraken_api_key, kraken_api_secret, kraken_config, kraken_timeout, least_difference, mandrill, mandrill_client, mandrill_config, message, price_warn, sell, sell_price;
  ...
  sell = function(difference, volume_long, volume_short) {
    return get_market_price(function(error, prices) {
      var timestamp;
      if (prices.market_sell <= (buy_price + (buy_price * fee_percent / 100)) && price_warn === false) {
